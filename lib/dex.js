...
let loaded = new Promise(resolve => {
	...
	command = `cd lib && mkdir -p data && mkdir -p sim && mkdir -p mods && mkdir config && echo "exports.Formats = [];" > ./config/formats.js && ${command}`;

	childProcess.exec(command).on('exit', () => {
		Dex = require('./sim/dex.js');
		resolve(true);
	}).on('error', err => {
		console.log(`ERROR while loading sim data: ${err.stack}`);
		resolve(false);
	});
});
